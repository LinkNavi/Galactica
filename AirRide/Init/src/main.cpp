...
private:
  ...
  void mount_filesystems() {
    ...
    mount("tmpfs", "/run", "tmpfs", MS_NOEXEC | MS_NOSUID | MS_NODEV,
          "mode=0755");
  ...
  bool parse_service_file(const std::string &filepath) {
    ...
    while (std::getline(file, line)) {
      ...
      if (current_section == "Service") {
        ...
        else if (key == "exec_start")
          svc.exec_start = value;
        else if (key == "exec_stop")
          svc.exec_stop = value;
        else if (key == "tty")
          svc.tty_device = value;
        else if (key == "autostart")
          svc.autostart = is_true(value);
        else if (key == "parallel")
          svc.parallel = is_true(value);
        else if (key == "clear_screen")
          svc.clear_screen = is_true(value);
        else if (key == "foreground")
          svc.foreground = is_true(value);
        else if (key == "type") {
          if (value == "simple")
            svc.type = ServiceType::SIMPLE;
          else if (value == "forking")
            svc.type = ServiceType::FORKING;
          else if (value == "oneshot")
            svc.type = ServiceType::ONESHOT;
        } else if (key == "restart")
        ...
        if (key == "requires" || key == "after") {
          std::vector<std::string> &target = (key == "requires") ? svc.
                                                                     requires
                                                                 : svc.after;
  ...
  void load_services() {
    ...
    std::cout << "[AirRide] " << services.size() << " services loaded"
              << std::endl;
  }
    ...
    for (int i = 0; i < timeout_sec * 10; i++) {
      {
        ...
        if (it != services.end()) {
          ...
          if (state == ServiceState::FAILED)
            return;
          if (type == ServiceType::ONESHOT && state == ServiceState::STOPPED)
            return;
        }
      }
      usleep(100000);
  ...
  void setup_control_socket() {
    ...
    // Switch to epoll instead of O_NONBLOCK polling
    epoll_fd = epoll_create1(0);
    if (epoll_fd == -1) {
      // fallback: keep old nonblock behavior
      fcntl(control_socket, F_SETFL,
            fcntl(control_socket, F_GETFL, 0) | O_NONBLOCK);
      return;
    }
  ...
  void handle_control_commands() {
    ...
    if (epoll_fd != -1) {
      ...
      // 2000ms timeout so reap_zombies and pending restarts still run
    ...
    if (n > 0) {
      ...
      if (cmd == "start")
        ...
        response = stop_service(svc_name) ? "OK\n" : "FAILED\n";
      else if (cmd == "restart") {
        stop_service(svc_name);
        usleep(500000);
        response = start_service(svc_name) ? "OK\n" : "FAILED\n";
      } else if (cmd == "status")
        response = get_service_status(svc_name);
      else if (cmd == "list")
  ...
  void reap_zombies() {
      ...
      for (auto &[name, svc] : services) {
        if (svc.pid == pid) {
          ...
          if (svc.restart_on_failure && svc.failures < 10) {
            svc.failures++;
            time_t when = time(nullptr) + svc.restart_delay;
            pending_restarts.push_back({name, when});
          }
          break;
        }
      }
    ...
    // Process pending restarts — no threads needed
    time_t now = time(nullptr);
    pending_restarts.erase(std::remove_if(pending_restarts.begin(),
                                          pending_restarts.end(),
                                          [&](const PendingRestart &r) {
                                            if (now >= r.when) {
                                              start_service(r.name);
                                              return true;
                                            }
                                            return false;
                                          }),
  ...
  void start_autostart_services() {
    ...
    // REMOVED: usleep(500000) — no reason to stall before getty
    ...
    if (!tty_services.empty()) {
      ...
      std::cout << "[AirRide] No TTY services, starting emergency shell"
                << std::endl;
      start_service("shell");
    }
  }
...
public:
  AirRide() { signal(SIGCHLD, SIG_DFL); }
  ...
  void run() {
    ...
    while (running) {
        handle_control_commands(); // blocks up to 2000ms via epoll_wait, or returns fast
        reap_zombies();
    }
