...
static int setup_tty(const char *tty_device) {
  ...
  if (!tty_device || tty_device[0] == '\0') {
    ...
    strncpy(g_tty_name,
            ttyname(STDIN_FILENO) ? ttyname(STDIN_FILENO) : "console",
            sizeof(g_tty_name) - 1);
    return 0;
  }
...
static int is_valid_username(const char *username) {
  ...
  if (!((username[0] >= 'a' && username[0] <= 'z') ||
        (username[0] >= 'A' && username[0] <= 'Z') || username[0] == '_')) {
    return 0;
  }
...
static int authenticate_user(const char *username, const char *password) {
  ...
  if (strcmp(encrypted, hash) == 0) {
    ...
    syslog(LOG_WARNING, "Failed login attempt for: %s on %s", username,
           g_tty_name);
    sleep(DELAY_AFTER_FAIL);
  }
...
static int setup_environment(struct passwd *pwd) {
  ...
  if (strncmp(g_tty_name, "tty", 3) == 0 && g_tty_name[3] >= '1' &&
      g_tty_name[3] <= '9') {
    setenv("DISPLAY", ":0", 1);
  }
...
static void start_shell(struct passwd *pwd) {
  ...
  syslog(LOG_ERR, "Failed to execute shell %s for user: %s", shell,
         pwd->pw_name);
  exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
  ...
  if (tty_device) {
    ...
    if (current_tty) {
      ...
      strncpy(g_tty_name, name ? name + 1 : current_tty,
              sizeof(g_tty_name) - 1);
    }
  }
